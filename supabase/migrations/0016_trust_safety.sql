...
--
-- Idempotent: safe to run repeatedly (guards on types/tables/policies), so a
-- partial earlier run can be completed by re-running this file.
--
...
-- functions (pattern from 0014): RLS does not re-evaluate inside a
-- security-definer function, avoiding recursion + grant complexity.
...
create index if not exists reports_target_gig_idx   on public.reports (target_gig_id)  where target_gig_id  is not null;
create unique index if not exists reports_unique_user_target on public.reports (reporter_id, target_user_id) where target_type = 'user';
create unique index if not exists reports_unique_gig_target  on public.reports (reporter_id, target_gig_id)  where target_type = 'gig';
...
returns boolean language sql stable security definer set search_path = public as $$
  select coalesce(
    (select (restrictions ->> p_key)::timestamptz > now() from public.profiles where id = auth.uid()),
    false)
$$;
...
returns boolean language sql stable security definer set search_path = public as $$
  select exists (
    select 1 from public.conversations c
     ...
     where c.id = p_convo and (c.user_a = auth.uid() or c.user_b = auth.uid()))
$$;
...
create policy "Users can file their own reports" on public.reports for insert with check (reporter_id = auth.uid());
drop policy if exists "Users can see their own reports" on public.reports;
create policy "Users can see their own reports" on public.reports for select using (reporter_id = auth.uid());

-- ──── RLS: blocks ───────────────────────────────────────────────────
alter table public.blocks enable row level security;
drop policy if exists "Users can see their own blocks" on public.blocks;
create policy "Users can see their own blocks" on public.blocks for select using (blocker_id = auth.uid());
drop policy if exists "Users can create their own blocks" on public.blocks;
create policy "Users can create their own blocks" on public.blocks for insert with check (blocker_id = auth.uid() and blocker_id <> blocked_id);
drop policy if exists "Users can remove their own blocks" on public.blocks;
create policy "Users can remove their own blocks" on public.blocks for delete using (blocker_id = auth.uid());
...
create policy "Authenticated users can post their own gigs"
  ...
  with check (auth.uid() = owner_id and not public.is_restricted('cannot_post_until'));
...
create policy "Participants can send messages as themselves"
  ...
  with check (
    sender_id = auth.uid()
    and conversation_id in (select id from public.conversations where user_a = auth.uid() or user_b = auth.uid())
...
declare me uuid := auth.uid(); a uuid; b uuid; convo_id uuid;
begin
  if me is null then raise exception 'Not authenticated'; end if;
  if other_user is null or other_user = me then raise exception 'Invalid recipient'; end if;
  if not exists (select 1 from public.profiles where id = other_user) then raise exception 'Recipient not found'; end if;
  if exists (select 1 from public.blocks where (blocker_id = me and blocked_id = other_user) or (blocker_id = other_user and blocked_id = me)) then
  ...
  if convo_id is null then insert into public.conversations(user_a, user_b) values (a, b) returning id into convo_id; end if;
  return convo_id;
end;
$$;
